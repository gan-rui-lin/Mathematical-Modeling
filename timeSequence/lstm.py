...
def main(data: pd.DataFrame, target_column: str, index_column: str, time_steps: int):
    ...
    print("数据预处理: ")
    print(
        f"原始数据范围:[{np.min(np.array(value)):.4f}, {np.max(np.array(value)):.4f}]"
    )
    ...
    model = build_lstm_model(
        input_shape, n_feature=n_feature, units=[50, 50], dropout_rate=0.2
    )
    ...
    future_dates = pd.date_range(
        last_date + pd.Timedelta(days=1), periods=future_steps, freq="D"
    )
    ...
    plt.axvline(
        x=last_date, color="gray", linestyle="--", alpha=0.7, label="Prediction Start"
    )
...
if __name__ == "__main__":
    ...
    data = pd.DataFrame({"Date": years, "Tax_Revenue": tax_revenue})
